...
w = symbols('w',real=True) # need real=True for numerator substitution to work

def define_problem(U,t_1,N_up,N_down,V):
    mu = U/2
    operator_spin = "up"
    t_0 = 0
    t_2 = 0
...
def plot_time_evolution(problem,n,sampling_period): 
...
def self_energy_weights(problem,denominator_roots_for_all_k_n,numerators):
    ...
    for integer_n,k_n in enumerate(problem.allowed_wavevectors):
        ...
        for root in denominator_roots:
            ...
            print("numerator weight",numerator_weight)
...
def plot_imaginary_self_energy(problem, denominator_roots_for_all_k_n, self_energy_weights_for_all_k_n):
    ...
    for integer_n,k_n in enumerate(problem.allowed_wavevectors):
        ...
        print(weights,type(weights),len(poles),len(weights))
...
def compute_kinetic(problem,all_minus_weights):
    ke_sum_2 = 0
...
def compute_potential(problem,all_denominator_roots,all_weights, greater_weights_for_all_k_n, lesser_weights_for_all_k_n, w_values_greater_for_all_k_n, w_values_lesser_for_all_k_n,numerators,denominators):
    sum_1 = 0
    for n,k_n in enumerate(problem.allowed_wavevectors):
        ...
        for index,root in enumerate(all_denominator_roots[n]):
            ...
            if root < 0:
                ...
                Re_G = real_green(n, problem, greater_weights_for_all_k_n, lesser_weights_for_all_k_n, w_values_greater_for_all_k_n, w_values_lesser_for_all_k_n)
                Re_G_lamdified = lambdify(w, Re_G, modules=['numpy'])
    ...
    sum_2 = 0
    for n,k_n in enumerate(problem.allowed_wavevectors):
        g_weights = lesser_weights_for_all_k_n[n]
        for l,root in enumerate(w_values_lesser_for_all_k_n[n]):

            sigma = numerators[n]/denominators[n]
...
def compute_ground_statte_energy(problem,mu,N_up,N_down,all_minus_weights,all_denominator_roots,all_weights, greater_weights_for_all_k_n, lesser_weights_for_all_k_n, w_values_greater_for_all_k_n, w_values_lesser_for_all_k_n,numerators,denominators):
    ke_sum_2 = compute_kinetic(problem,all_minus_weights)
    sum_1,sum_2 = compute_potential(problem,all_denominator_roots,all_weights, greater_weights_for_all_k_n, lesser_weights_for_all_k_n, w_values_greater_for_all_k_n, w_values_lesser_for_all_k_n,numerators,denominators)
    ground_state_energy = mu*N_down-mu*(N_up+N_down)+re( 2*ke_sum_2 + (sum_1 + sum_2) )
...
def go(U,t,n,sampling_period):

    N_up = 2
    N_down = 2
    V = 4
    problem, mu = define_problem(U,t,N_up,N_down,V)       

    # 0) Time Evolution
    imaginary_greater_vs_time, imaginary_lesser_vs_time, n, sampling_f = plot_time_evolution(problem,n,sampling_period)
    ...
    #5 ) Ground State Energy Computation
    kinetic = compute_kinetic(problem,lesser_weights_for_all_k_n)
    print("<T>", kinetic)
    potential = compute_potential(problem,denominator_roots_for_all_k_n,self_energy_weights_for_all_k_n,greater_weights_for_all_k_n, lesser_weights_for_all_k_n, w_values_greater_for_all_k_n, w_values_lesser_for_all_k_n,numerators,denominators)
    print("<U>",potential)
    computed_ground_state_energy = compute_ground_statte_energy(problem,mu,N_up,N_down,lesser_weights_for_all_k_n,denominator_roots_for_all_k_n, self_energy_weights_for_all_k_n, greater_weights_for_all_k_n, lesser_weights_for_all_k_n, w_values_greater_for_all_k_n, w_values_lesser_for_all_k_n,numerators,denominators)
    print("computed <H>",computed_ground_state_energy)
    print("exact <H>",problem.hubbard_N.ground_state_energy)

    plt.scatter(U,computed_ground_state_energy,marker='d',color='blue',s=10)
